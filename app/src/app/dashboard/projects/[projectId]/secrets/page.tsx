...
import { Eye, EyeOff, Copy, Plus, Trash2, Download, Loader2, ArrowLeft, KeyRound } from "lucide-react";
    ...
    <div className="container mx-auto px-6 py-8 max-w-5xl">
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-slate-800/30 rounded-lg p-4 border border-slate-700 animate-pulse">
        ...
        <div className="space-y-3">
          {secrets.map((secret) => (
            ...
            >
              ...
              {secret.notes && (
                <p className="text-xs text-slate-500 mt-2">{secret.notes}</p>
              )}
            </div>
          ))}
        </div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Add Secret">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={createLoading} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-lg text-sm font-medium flex items-center gap-2">
      ...
      <Modal open={!!editingSecret} onClose={() => setEditingSecret(null)} title="Edit Secret">
        <form onSubmit={handleUpdate} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setEditingSecret(null)} className="px-4 py-2 text-slate-300 hover:text-white text-sm">Cancel</button>
            <button type="submit" disabled={editLoading} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-lg text-sm font-medium flex items-center gap-2">
