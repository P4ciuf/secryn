...
import { loginAction, registerAction, logoutAction, verifyAccountAction } from "../actions";
...
describe("verifyAccountAction", () => {
  ...
  it("calls verifyAccount with the current user id", async () => {
    const mockAuthService = await AuthService.Instance(null);
    ...
    expect(result.success).toBe(true);
    expect(mockAuthService.verifyAccount).toHaveBeenCalledWith("user-1");
  });
