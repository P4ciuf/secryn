 ...
 *
 * Each instance is scoped to a single request and emits audit-log
...
export class AuthService {
  ...
  /**
   * @param req         - The incoming request, used to inspect cookies,
   ...
   */
  static async Instance(req: Request): Promise<AuthService> {
  ...
  /**
   * Resolves the owner of an encrypted API key and validates its existence.
   * Used as the API-key alternative to JWT cookie authentication.
   */
  ...
  /**
   * Authenticates the current request using either the JWT cookie or an
   * {@code api-key} header (only on /secrets routes). Returns a discriminated
   * union so callers can distinguish user sessions from API key access.
   *
   * @returns A user payload with {@code type: "USER"} or an API key with
   *          {@code type: "APIKEY"}.
   * @throws {AppError} Unauthorized when neither authentication method succeeds.
   */
  ...
  /** Reads the password-reset email template from disk and sends it asynchronously. */
